  ...
  */
  function WriteToLog( )
  {
    ...
    $sql = "INSERT INTO log (timestamp,ip,accountid,username,activityid,fechayhora) 
            VALUES(:time,:ip,:acc,:user,:act, '$ahora_ymd')";
    
    try {
        $resultp = $this->pDB->prepare($sql);
        $datadb=array(':time'=>$time,':ip'=>$ip,':acc'=>$this->mAccountId,':user'=> $this->mUserName, ':act'=>$this->mActivityId);
        $resultp->execute($datadb);
        
    }catch(PDOException  $e ){
        $this-> SetErrorMessage('Error de lectura wl','');
        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura wl','');
        return false;
    }
...
//     }
    ...
    foreach (array('HTTP_CLIENT_IP', 'HTTP_X_FORWARDED_FOR', 'HTTP_X_FORWARDED', 'HTTP_X_CLUSTER_CLIENT_IP', 'HTTP_FORWARDED_FOR', 'HTTP_FORWARDED', 'REMOTE_ADDR') as $key)
    {
        ...
        {
            foreach (explode(',', $_SERVER[$key]) as $ipv)
            {
                if (filter_var($ipv, FILTER_VALIDATE_IP) !== false)
                {
                    $ip = $ipv;
                    break;
                }
  ...
  */
  function SendSelectLogDate($FormElements)
  {
    ...
    $timeSelection['86400']   = '1 d�a';
    $timeSelection['172800']  = '2 d�as';
    $timeSelection['259200']  = '3 d�as';
    $timeSelection['345600']  = '4 d�as';
    $timeSelection['432000']  = '5 d�as';
    $timeSelection['518400']  = '6 d�as';
  ...
  */
  function ListLog($beginTime,$endTime='')
  {
    ...
    if ($endTime != '') $endTime = intval($endTime);
    ...
    $number=0;
      try {
          ...
          $this-> SetErrorMessage('Error de lectura ll','');
          if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura ll','');
          return false;
      }
...
//    $number=(int)$result->fields("totalrecord");#record results selected from db
    ...
    try {
        ...
        while($row2=$resultp2->fetch(PDO::FETCH_OBJ)) {
            ...
            $timestamp = date("F jS, Y -- g:ia",$row2->timestamp);
            
            $passtru->SetValue($value=$timestamp);
            $passtru->SetClass("DataTD");
            
            $this->myForm-> AddFormElementToNewLine($passtru);
            $passtru->SetValue($row2->ip);
            
            $this->myForm-> AddFormElement($passtru);
            
            $accountName = $Account-> GetAccountName($row2->accountid);
            $passtru->SetValue($accountName);
            
            $this->myForm-> AddFormElement($passtru);
            
            $passtru->SetValue($row2->username);
            $this->myForm-> AddFormElement($passtru);
            
            $activity = $Activity-> GetDescription($row2->activityid);
