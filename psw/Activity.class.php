	...
	*/
	function GetDescription($key)
	{
	    ...
	    try {
	        ...
	        $this-> SetErrorMessage('Error de lectura gd','');
	        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura gd','');
	        return false;
	    }
...
// 		return $result->fields("description");
	...
	*/
	function GetActivityIdByName($name)
	{
	    ...
	    $sql = "SELECT * FROM activity WHERE description=:key";
	    try {
	        $resultp2 = $this->pDB->prepare($sql);
	        $datadb=array(':key'=>$name);
	        $resultp2->execute($datadb);
	        while($row2=$resultp2->fetch(PDO::FETCH_OBJ)) {
	            return $row2->activityid;
	        }
	    }catch(PDOException  $e ){
...
// 		return $result->fields("activityid");
	...
	*/
	function DeleteActivity($activityId)
	{
	    ...
	    $sql = "DELETE FROM activity WHERE activityid=:key";
	    try {
	        $resultp2 = $this->pDB->prepare($sql);
	        $datadb=array(':key'=>$activityId);
	        $resultp2->execute($datadb);
	    }catch(PDOException  $e ){
		...
		try {
		    ...
		    while($row2=$resultp2->fetch(PDO::FETCH_OBJ)) {
		        ...
		        if ($this->mySecurity-> isAllowedTo(16))
		            $edit = "\n<a class=\"".$_SESSION["CSS"]."LinkButton\" href=\"activityModify.php?activityId=".$row2->activityid."&mode=edit\">Editar</a>";
		            # Delete
		            // 				if ($this->mySecurity-> isAllowedTo('delete activity'))
		            if ($this->mySecurity-> isAllowedTo(15))
		                $delete = "\n<a class=\"".$_SESSION["CSS"]."LinkButton\" href=\"activityModify.php?activityId=".$row2->activityid."&mode=delete\">Borrar</a>";
		                
		                $passtru = new PassTru();
		                
		                $passtru->SetValue($value=$row2->activityid);
		                $passtru->SetClass("FieldCaptionTD");
		                
		                $this->myForm-> AddFormElementToNewLine($passtru);
		                
		                $passtru = new PassTru(); //Pablo 280809
		                $passtru->SetValue($value=$row2->description);
