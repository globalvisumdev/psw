...
class Table
{
    ...
    * @return    int        Returns the current number of rows
    */
    function AddPassTru ($content)
    {
        $this->passTru[$this->passTruCounter++] = $content;
    }
    ...
    * @param    string    $class
    */
    function SetCellClass ( $row, $col, $class )
    {
        if (strlen($class))
				$this->table[$row][$col]["class"] = $class;
    }
    ...
    */
    ...
    {
        if (strlen($class))
				$this->default_settings['td']['class'] = $class;
    }
    ...
    * @return    void
		*/
		...
		{
				if (strlen($overcolor) == 6)
					$this->tableTRMouseoverColor = "#".$overcolor;
				elseif (strlen($overcolor) == 7)
					$this->tableTRMouseoverColor = $overcolor;
				elseif (strlen($overcolor) < 6)
					$this->tableTRMouseoverColor = "#D2D2D2";
				
				if (strlen($outcolor) == 6)
					$this->tableTRMouseoutColor = "#".$outcolor;
				elseif (strlen($outcolor) == 7)
					$this->tableTRMouseoutColor = $outcolor;
				elseif (strlen($outcolor) < 6)
					$this->tableTRMouseoutColor = "#cccccc";

				$this->tableTRMouseoverColorStartingRow = $startingRow;
		}
    ...
    */
    function CompileTable ()
    {
        ...
        {						
						...
						if (is_string($this->tableTRMouseoverColor) and
								$this->tableTRMouseoverColorStartingRow )
						{
							if ($row >= $this->tableTRMouseoverColorStartingRow)
							{
								$mouseover = "onmouseover=\"if(document.layers) { document.layers['".TR_Layer.$row."'].bgColor='$this->tableTRMouseoverColor' } else { if(document.all) { document.all['".TR_Layer.$row."'].style.background='$this->tableTRMouseoverColor' } }\" onmouseout=\"if(document.layers) { document.layers['".TR_Layer.$row."'].bgColor='$this->tableTRMouseoutColor' } else { if(document.all) { document.all['".TR_Layer.$row."'].style.background='$this->tableTRMouseoutColor' } }\"";

								$content .= "\t<tr id=\"TR_Layer$row\" $mouseover>\n";
							}
							else
								$content .= "\t<tr>\n"; 
						}
            ...
            $content .= "\t</tr>\n";
				...
				#here we will print all the passTru array before closing the table
	
				$content .= "\n\n\t<!-- Start of passtru values -->\n";

				foreach ($this->passTru as $value) 
				{
				    $content .= "\t$value\n";
				}
