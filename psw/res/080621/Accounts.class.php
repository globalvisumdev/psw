...
class Accounts extends MyDatabase {
	...
	function GetAccount($key) {
		...
		try {
		    ...
		    if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura gfn','');
		    return false;
		}
	...
	function AddGroupsToAccount($FormElements) {
		if (count ( $FormElements ["availablegroups"] )) {
			...
			foreach ( $FormElements ["availablegroups"] as $key => $groupid ) {
			    ...
			    try {
			        $datadb=array(':grupo'=>$groupid,':account'=>$_GET ['accountId']);
			        $resultp->execute($datadb);
			        
			    }catch(PDOException  $e ){
			        $this->pDB->rollBack();
			        $this-> SetErrorMessage('Error de lectura agta','');
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura agta','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountgroups"] )) {
			...
			foreach ( $FormElements ["accountgroups"] as $key => $groupid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura rgfa','');
			        return false;
			    }
			}
...
// 			$this->gDB->CommitTrans ();
	...
	Function AddAccount($FormElements) {
	    ...
	    try {
	        ...
	        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aa','');
	        return false;
	    }
...
//			$sql = "SELECT cliente_nombre,hierarchy,a.accountid,a.* FROM megacontrol.accounts a " . " LEFT JOIN megacontrol.groupaccounts ga ON a.accountid=ga.accountid " . " LEFT JOIN megacontrol.groups g ON g.groupid=ga.groupid " . " LEFT JOIN megacontrol.cliente ON a.cliente_id=cliente.cliente_id " . " WHERE (hierarchy >=" . $_SESSION ['myHierarchy'] . " or hierarchy is null)" . " GROUP BY accountid" . " ORDER BY firstname,lastname";
			...
			$sql = "SELECT cliente_nombre,hierarchy,a.accountid,a.firstname,a.lastname,a.username,group_concat(groupname) grupos FROM megacontrol.accounts a 
                ...
                WHERE (hierarchy >= :hier or hierarchy is null) AND a.accountid > 1 AND a.cliente_id =:cli 
		...
		<td>
			<table border="0" cellspacing="0" cellpadding="4">
				<tr class="thead" align="center">
					...
					<td class="micronautaColumnTD">Grupo/s</td>
		...
		try {
		    ...
		    while($row=$resultp->fetch(PDO::FETCH_OBJ)) {
		        ...
		        if ($this->mySecurity->isAllowedTo ( 12 )) {
		            $edit = "\n<a class=\"" . $_SESSION ["CSS"] . "LinkButton\" href=\"accountsModify.php?accountId=" . $row->accountid;
            ...
            <tr align="center" valign="top">
					<td class="micronautaDataTD" align="left"><? echo $row->firstname;?> <? echo $row->lastname;?></td>
					<td class="micronautaDataTD" align="left"><? echo $row->username;?></td>
					<td class="micronautaDataTD" align="left"><? echo $row->grupos;?></td>
...
<?php
	...
	function SendAccountsForm($FormElements, $alsoSendAccountsGroupForm = false, $basico = false) {
		...
		$myForm->SetFormHeader ( "Información de Usuarios" );
		
		if ($_SESSION ['myHierarchy'] == 1) {
			$myForm->AddFormElementToNewLine ( new Label ( "lb1", "Cliente :" ) );
			// El usuario "admin" (jerarquía 1) es el único que puede asignar un usuario a un cliente determinado
    ...
    ";
		...
		if ("EDIT" == strtoupper ( $_GET ['mode'] )) {
			...
			$myForm->AddFormElementToNewLine ( new Label ( "lb1", "Último intento de acceso:" ) );
...
//				if ($this->mySecurity->isAllowedTo ( 'Modify Account' ) or $basico == true) {
			...
			default :
				...
				if ($this->mySecurity->isAllowedTo ( 10 )) {
					$buttons = new ObjectArray ( "buttons" );
					$buttons->AddObject ( new SubmitButton ( "B_add_submit", "Añadir nuevo Usuario" ) );
	...
	function AccountsGroupForm($FormElements, $formTagRequired = TRUE) {
		...
		$myForm->SetFormHeader ( "Información de Grupos de Usuarios" );
...
// 		$_SESSION ['available_groups'] = $available_Groups;
	...
	function SendExpiredForm($FormElements) {
		...
		$myForm->SetCSS ( $_SESSION ["CSS"] );
		$myForm->SetEmptyCells ( true );
		$myForm->SetFormHeader ( "Selección de Lista de Usuarios" );
		
		$myForm->AddFormElementToNewLine ( new Label ( "lb1", "Ver sólo usuarios desactivados" ) );
		...
		$myForm->AddFormElementToNewLine ( new Label ( "lb1", "Ver sólo usuarios activos" ) );
			...
			if ($FormElements ["oldpassword"] != "" and $FormElements ["blanquear"] != "1")
				...
				// their password, or you have to know the old password to change it.
					...
					if (USE_MD5)
						...
						$count=0;
						$sql = "SELECT accountid FROM accounts WHERE accountid=:account AND password= :password";
						try {
						    $resultp2 = $this->pDB->prepare($sql);
...
// 					$sql = "SELECT accountid FROM accounts WHERE accountid=$accountid " . "AND password=" . ToSQL ( $password, "text" );
...
// 					$sql = "SELECT accountid FROM accounts WHERE accountid=$accountid limit 1";
...
// 			$sql = "SELECT hierarchy,a.accountid,a.* FROM megacontrol.accounts a 
//                 LEFT JOIN megacontrol.groupaccounts ga ON a.accountid=ga.accountid 
//                 LEFT JOIN megacontrol.groups g ON g.groupid=ga.groupid 
//                 WHERE (hierarchy >=" . intval($_SESSION ['myHierarchy']) . " or hierarchy is null) 
//                  AND a.accountid > 1 AND a.cliente_id ='" . intval($_SESSION ['cliente_id']) . "' 
		...
		if (count ( $FormElements ["allactions"] )) {
			...
			foreach ( $FormElements ["allactions"] as $key => $actionid) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aaa','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountactions"] )) {
			...
			foreach ( $FormElements ["accountactions"] as $key => $actionid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura raa','');
			        return false;
			    }
			}
...
//			    echo '<br>'.$empresaid." ".var_dump($_SESSION ['available_empresas']);
			...
			foreach ( $FormElements ["allempresas"] as $key => $empresaid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aae','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountempresa"] )) {
			...
			foreach ( $FormElements ["accountempresa"] as $key => $empresaid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura rae','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["alljurisdicciones"] )) {
			...
			foreach ( $FormElements ["alljurisdicciones"] as $key => $jurisdiccionid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aaj','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountjurisdiccion"] )) {
			...
			foreach ( $FormElements ["accountjurisdiccion"] as $key => $jurisdiccionid ) {
			    ...
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura raj','');
			        return false;
			    }
			}
	    ...
	    if (count ( $FormElements ["allvehiculos"] )) {
	        ...
	        foreach ( $FormElements ["allvehiculos"] as $key => $vehiculo_id ) {
	            try {
	                ...
	                if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aav','');
	                return false;
	            }
	        }
	    ...
	    if (count ( $FormElements ["accountvehiculo"] )) {
	        ...
	        foreach ( $FormElements ["accountvehiculo"] as $key => $vehiculo_id ) {
	            try {
	                ...
	                if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura rav','');
	                return false;
	            }
	        }
    ...
    if (count ( $FormElements ["allservicios"] )) {
        ...
        foreach ( $FormElements ["allservicios"] as $key => $servicio_id ) {
            try {
                ...
                if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aas','');
                return false;
            }
        }
    ...
    if (count ( $FormElements ["accountservicio"] )) {
        ...
        foreach ( $FormElements ["accountservicio"] as $key => $servicio_id ) {
            try {
                ...
                if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura ras','');
                return false;
            }
        }
		...
		if (count ( $FormElements ["allclientesclientes"] )) {
			...
			foreach ( $FormElements ["allclientesclientes"] as $key => $clienteid ) {
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aacc','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountclientecliente"] )) {
			...
			foreach ( $FormElements ["accountclientecliente"] as $key => $clienteid ) {
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura racc','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["allclientesequivales"] )) {
			...
			foreach ( $FormElements ["allclientesequivales"] as $key => $clienteid ) {
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aace','');
			        return false;
			    }
			}
		...
		if (count ( $FormElements ["accountclienteequivale"] )) {
			...
			foreach ( $FormElements ["accountclienteequivale"] as $key => $clienteid ) {
			    try {
			        ...
			        if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura race','');
			        return false;
			    }
			}
