    ...
    if (count($FormElements["allactions"]))
    {
      ...
      foreach( $FormElements["allactions"] as $key=>$actionid)
      {
          try {
              ...
              if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura aga','');
              return false;
          }
      }
    ...
    if (count($FormElements["groupactions"]))
    {
      ...
      foreach( $FormElements["groupactions"] as $key=>$actionid)
      {
          try {
              ...
              if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura rga','');
              return false;
          }
      }
    ...
    if (count($FormElements["allEmpresasExceptGroup"]))
    {
      ...
      foreach( $FormElements["allEmpresasExceptGroup"] as $key=>$empresaid)
      {
          try {
              ...
              if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura age','');
              return false;
          }
      }
    ...
    if (count($FormElements["groupempresa"]))
    {
      ...
      foreach( $FormElements["groupempresa"] as $key=>$empresaid)
      {
          try {
              ...
              if ($_SESSION['IS_ERROR_REPORTING']) $this-> EchoError(  'Error de lectura rge','');
              return false;
          }
      }
...
//       }
  ...
  */
  function ListGroups( )
  {
      ...
      $sql = "SELECT * FROM groups WHERE hierarchy >= :hier and (cliente_id=:cli or cliente_id = 0) ORDER BY hierarchy,groupname";
...
//   				GROUP BY accountid  ORDER BY firstname,lastname";
  		//// ." AND a.accountid=ga.accountid AND g.groupid=ga.groupid " . $userExtra
  	    $sql = "SELECT cliente_nombre,hierarchy,a.* FROM megacontrol.accounts a
  				LEFT JOIN megacontrol.groupaccounts ga ON a.accountid=ga.accountid
  				LEFT JOIN megacontrol.groups g ON g.groupid=ga.groupid
  				LEFT JOIN megacontrol.cliente ON a.cliente_id=cliente.cliente_id
  ...
  <table class="micronautaFormTABLE" cellspacing="1" cellpadding="5"
      ...
      <?php
  		...
  		try {
  		    $resultp2 = $this->pDB->prepare($sql);
  		    $datadb=array(':hier'=>$_SESSION ['myHierarchy'],':grupo'=>$_GET[groupId],':cli'=>$_SESSION ['cliente_id']);
  		    $resultp2->execute($datadb);
  		    while($row2=$resultp2->fetch(PDO::FETCH_OBJ)) {
  		        $edit = "&nbsp;";
  		        $delete = "&nbsp;";
  		        ...
  		        if ($this->mySecurity->isAllowedTo ( 12 )) {
  		            $edit = "\n<a class=\"" . $_SESSION ["CSS"] . "LinkButton\" href=\"accountsModify.php?accountId=" . $row2->accountid;
  		        ...
  		        if ($this->mySecurity->isAllowedTo ( 11 ) and $row2->accountid != 1 and $row2->accountid != $_SESSION ['myAccount']) {
  		            $delete = "\n<a class=\"" . $_SESSION ["CSS"] . "LinkButton\" href=\"accountsModify.php?accountId=" . $row2->accountid;
  		            $delete .= "&mode=delete&selectlisttype=$selectlisttype\">Borrar</a>";
  		        }
  		        ?>
              <tr align="center" valign="top">
  					<td class="micronautaDataTD" align="left"><? echo $row2->firstname;?> <? echo $row2->lastname;?></td>
  					<td class="micronautaDataTD" align="left"><? echo $row2->username;?></td>
