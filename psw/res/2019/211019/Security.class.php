...
//     }
    ...
    $accountid=0;
    $sql = "SELECT * FROM accounts,cliente WHERE username = :user  AND password=:pass and cliente.cliente_id = accounts.cliente_id";
    try {
  ...
  function ValidarPassword($username,$clave,&$error_clave,$clave_rep= ""){
      ...
      if ($clave != $clave_rep and $clave_rep > "") {
          $error_clave = 'La repeticion de la contraseña nueva no coincide';
          return false;
      }
      
      $this->mUsername = ToSQL($value=htmlspecialchars($username),$type='text_seguro');
      #echo 'USE_MD5: '.USE_MD5."<br>";
      if ($md5)
          $password = ToSQL($value=md5(htmlspecialchars($clave)),$type='text');
        else
          $password = ToSQL($value=htmlspecialchars($clave),$type='text');
              #echo '$password: '.$password.'<br>';
              ...
              if($result->fields("accountid") > 0) {
                  if ($result->fields("password") == $password) {
                      $error_clave = 'La contraseña nueva no puede ser igual a la actual';
                      return false;
                  }
              }
